...
const SHAPES = [
  ...
  // 背景ワールド用の大型形状。内角素材は O7(tr) と O15(bl) しかなく、
  // 回転・反転も使わないため、凹みは右上と左下だけ（左上から右下へ降りる階段）に限られる。
  {
    name: "ridge-wide", width: 448, height: 160,
    inside: (x, y) => x >= 0 && x < 448 && y >= 0 && y < 160,
    top: [[0, 448, 0]], bottom: [[0, 448, 160]], left: [[0, 160, 0]], right: [[0, 160, 448]],
    corners: [["tl", 0, 0], ["tr", 448, 0], ["br", 448, 160], ["bl", 0, 160]],
  },
  {
    name: "bluff-tall", width: 192, height: 384,
    inside: (x, y) => x >= 0 && x < 192 && y >= 0 && y < 384,
    top: [[0, 192, 0]], bottom: [[0, 192, 384]], left: [[0, 384, 0]], right: [[0, 384, 192]],
    corners: [["tl", 0, 0], ["tr", 192, 0], ["br", 192, 384], ["bl", 0, 384]],
  },
  {
    name: "terrace", width: 448, height: 288,
    inside: (x, y) => x >= 0 && x < 448 && y >= 0 && y < 288
      && ((x < 256 && y < 160) || (x >= 128 && y >= 96)),
    top: [[0, 256, 0], [256, 448, 96]], bottom: [[0, 128, 160], [128, 448, 288]],
    left: [[0, 160, 0], [160, 288, 128]], right: [[0, 96, 256], [96, 288, 448]],
    corners: [["tl", 0, 0], ["tr", 256, 0], ["tr", 448, 96], ["br", 448, 288], ["bl", 128, 288], ["bl", 0, 160]],
    innerCorners: [["tr", 256, 96], ["bl", 128, 160]],
  },
  {
    name: "headland", width: 576, height: 320,
    inside: (x, y) => x >= 0 && x < 576 && y >= 0 && y < 320
      && ((x < 320 && y < 192) || (x >= 160 && y >= 112)),
    top: [[0, 320, 0], [320, 576, 112]], bottom: [[0, 160, 192], [160, 576, 320]],
    left: [[0, 192, 0], [192, 320, 160]], right: [[0, 112, 320], [112, 320, 576]],
    corners: [["tl", 0, 0], ["tr", 320, 0], ["tr", 576, 112], ["br", 576, 320], ["bl", 160, 320], ["bl", 0, 192]],
    innerCorners: [["tr", 320, 112], ["bl", 160, 192]],
  },
];
