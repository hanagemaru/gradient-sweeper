...
- **クリスタルはUIパネルに重ねない（セル系モチーフは重ねてよい）。**
  ...
  スマホ5サイズ × 本番4ページで `.stack` を実測した和が
  `x[1241,1639] y[476,1082]`。余裕を足した `x[1233,1647] y[468,1094]` を
  キープアウトとして、クリスタルだけ弾いている。
  この幅（414px）は最小スマホ幅 375px より広い。つまり最小画面ではパネルの
  左右に逃げ場が無く、クリスタルは上下の帯にしか置けない。
  そのため**島を撒く前に**、キープアウト下の帯へ `accent-small` を1つ確保している。
  パネル実測値は `scripts/generate-background-world.mjs` のコメントに残してある。
