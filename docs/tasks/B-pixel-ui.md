...
## 完了（PR #10）とスコープ外の申し送り

PR #10 はマージ済み。ホーム・TA選択・リザルト・ランキングの4ページが
固定キャンバス＋レターボックス方式の CRYSTAL FIELD デザインへ移行済み。

以下の2点はこのタスクの対象外として持ち越した。

- **`/game`（ゲーム画面）はレスポンシブのまま、今回のレターボックス化の対象外。**
  `src/app/game/`・`src/components/game/` はレーンAの担当かつ「触ってはいけない
  ファイル」なので、このタスクでは変更していない。着手するなら別タスクで。
- **`/result`（リザルト画面）は本来ゲーム画面の上にオーバーレイされるべきで、
  今回追加した背景ワールド（`PixelScene` の `.backdrop`）を使うべきではない
  かもしれない。** 現状は `/game` から `router.push("/result?...")` で完全に
  別ページへ遷移しており、`ResultContent` は他の3ページと同じ
  `<PixelScene>`（背景ワールド付き）を使っている。ゲーム盤面の上に重ねる設計に
  変えるなら、リザルトだけ背景無しの軽量な `PixelScene` バリアントにするか、
  ゲーム画面側でモーダル的に表示する作りに変える必要がある。詳細は
  `docs/tasks/H-result-overlay.md` を参照。
